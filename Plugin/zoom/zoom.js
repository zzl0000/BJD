	...
	e.fn.smartZoom = function (t) {
		...
		function E(t, r, i, s, o, u, a) {
			...
			if(f.transitionObject != null) {
				...
				else d[f.transitionObject.transform] = "translateX(" + c + "px) translateY(" + h + "px) scale(" + s / p.width + "," + o / p.height + ")"; if(a != null) {
...
function analyze(){
	...
	if($('.zoomModal').length == 0){
		$('body').append('<section class="T_modalWrap zoomModal"></section>');
	}
	
	var objs = $('.zoom-img');
	
	//将所有图片变为手形
	objs.find('img').css('cursor', 'pointer');
	
	for(var i =0; i < objs.length; i++){
		var spattr = 'zoomimg_' + i;
		$(objs[i]).attr('spattr', spattr);
		var imgs = $(objs[i]).find('img');
		
		if(imgs.length > 0){
			for(var j = 0; j < imgs.length; j++){
				$(imgs[j]).attr('i', j);
				$(imgs[j]).attr('p', spattr);					
				$(imgs[j]).bind('click',function(){
					
					var allImg = $("[spattr='"+$(this).attr('p')+"']").find('img');
					var current = parseInt($(this).attr('i'));
					
					var img = '';
					for(var i = 0; i < allImg.length; i++){
						if(i == current){
							img += '<img  src ="'+ $(allImg[i]).attr('src') + '" class="curzoom" style="visibility:hidden" />';
						}
					}
					
					//生成放大图片
					$('.zoomModal').append('<div class="zoomContainer" i="'+ $(this).attr('i') +'" p="'+ $(this).attr('p') +'" style="width:500px; height:500px;background-color: #ffffff">' + img + '</div>');
					...
					Tui.Page({
	            		...
	            		success:function(){
	            			//动态添加关闭按钮
	            			$('.zoomContainer').append('<button class="T_closeModal"></button>');
	            			
	            			//绑定鼠标事件
							$('.zoomContainer').bind('mouseover',function(){
								//显示左右按钮
								var allImg = $("[spattr='"+$(this).attr('p')+"']").find('img');
								var current = parseInt($(this).attr('i'));
								...
								if(current == 0){
									$(this).find('.zoomBtn.zoomRight').show();
								}else if(current == allImg.length - 1){
									$(this).find('.zoomBtn.zoomLeft').show();
								}else{
									$(this).find('.zoomBtn').show();
								}
							}).bind('mouseout',function(){
								//隐藏左右按钮
								$(this).find('.zoomBtn').hide();
							});
						
							setTimeout(function(){ 
								$('.zoomContainer').css('background-image','none');
								$('.zoomContainer').css('background-color','#ffffff');
								$('.curzoom').smartZoom({'containerClass':'zoomableContainer'});
								$('.curzoom').css('visibility','visible')
								
								$('.zoomContainer').append('<div class="zoomBtn zoomLeft"></div><div class="zoomBtn zoomRight"></div>');
								
								$('.zoomBtn').bind('click', function(){
									var container = $('.zoomContainer');
									
									var allImg = $("[spattr='"+container.attr('p')+"']").find('img');
									var current = parseInt(container.attr('i'));
									
									if($(this).hasClass('zoomLeft')){
										current--;
									}else{
										current++;
									}
									
									container.find('.zoomableContainer').remove();
									container.attr('i', current);
									container.prepend('<img  src ="'+ $(allImg[current]).attr('src') + '" class="curzoom" style="visibility:hidden" />');
									
									$('.curzoom').smartZoom({'containerClass':'zoomableContainer'});
									$('.curzoom').css('visibility','visible')
									
									container.find('.zoomBtn').hide();
									if(current == 0){
										$(this).find('.zoomBtn.zoomRight').show();
									}else if(current == allImg.length - 1){
										$(this).find('.zoomBtn.zoomLeft').show();
...
$(document).ready(function() {
	 ...
	 * @author   Tony
	 */
	Tui.include('/Templates/Default/Plugin/zoom/zoom.css','css');
});